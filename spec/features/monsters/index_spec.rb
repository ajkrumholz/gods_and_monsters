...
RSpec.describe 'Monster Index Page' do
  describe "Index Page" do 
    # let(:khorne) { God.create!(name: "Khorne", age: 20, immortal: false) }
    # let(:ursun) { God.create!(name: "Ursun", age: 20, immortal: false) }
    # let(:slaanesh) { God.create!(name: "Slaanesh", age: 20, immortal: false) }
    # require 'pry'; binding.pry
    # let(:bloodthirster) { khorne.monsters.create!(name: "Bloodthirster", strength_rank: 8.5, flying: true) }
    # let(:hellhound) { khorne.monsters.create!(name: "Hellhound", strength_rank: 5.5, flying: false) }
    # let(:elemental_bear) { ursun.monsters.create!(name: "Elemental Bear", strength_rank: 7.0, flying: false) }
    # let(:daemonette) { slaanesh.monsters.create!(name: "Daemonette", strength_rank: 3.5, flying: false) }
    # let(:daemon_priestess) { slaanesh.monsters.create!(name: "Daemon Priestess", strength_rank: 9.0, flying: true) }
